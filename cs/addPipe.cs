/*
Add Pipe:Indoor and Pipe:Outdoor Objects to the EnergyPlus IDF

Purpose

This DesignBuilder script runs before the EnergyPlus simulation and modifies the generated IDF file by:
- Creating insulated pipe constructions (Construction + Material layers)
- Inserting Pipe:Indoor and Pipe:Outdoor objects into specified plant Branch objects
- Updating PlantLoop outlet node references to match the new pipe outlet nodes

How to Use

1) Open BeforeEnergySimulation() and edit the "Configuration" section:
   - Define one or more pipe constructions via AddInsulatedPipeConstruction(...)
   - Call AddIndoorPipeToBranch(...) and/or AddOutdoorPipeToBranch(...) for each pipe you want to add
2) Ensure the Branch names and Zone names match exactly what exists in the IDF generated by DesignBuilder.
3) Run the DesignBuilder simulation; the script will edit the IDF before EnergyPlus executes.
